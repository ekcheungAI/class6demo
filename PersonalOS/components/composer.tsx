 ...
 return <section className="panel">
  ...
  <div style={{display:'grid',gap:12,marginBottom:16}}>
   <fieldset style={{border:'1px solid #e9e7ee',borderRadius:8,padding:12}}><legend>來源</legend>
    ...
    {(data?.social||[]).length>0&&<label style={{display:'block',marginTop:8}}><input type="radio" name="src" checked={pick.kind==='social'} onChange={()=>setPick({kind:'social',postId:data.social[0].post_id})}/> 追蹤帳戶嘅帖（Inspiration）：<select disabled={pick.kind!=='social'} value={pick.postId||''} onChange={e=>setPick({kind:'social',postId:e.target.value})}>{data.social.map((f:any)=><option key={f.post_id} value={f.post_id}>{f.platform} @{f.account} · {f.title}</option>)}</select></label>}
