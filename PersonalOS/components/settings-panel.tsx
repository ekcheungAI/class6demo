...
export default function SettingsPanel({token}:{token:string}){
 ...
 return <section className="panel settings" style={{marginTop:24}}>
  ...
  <p className="small-copy" style={{marginTop:12}}>裝 cron（功課／老師示範）：`vercel.json` 已登記 <code>/api/cron/run</code> 每日 01:00 UTC（09:00 HKT）；Vercel env 要有 <code>CRON_SECRET</code> 同 <code>AUTOPILOT_EMAIL</code>／<code>AUTOPILOT_PASSWORD</code>（runner 用你個 app 登入跑）。冇呢三條 = cron 唔會跑，唔會出事。</p>
 </section>;
}
