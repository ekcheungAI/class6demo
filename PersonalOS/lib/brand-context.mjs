...
export async function loadBrand(project=process.cwd(),vaultOverride=undefined){
  ...
  // private). Fall back to the DEMO brand (fixtures/brain-sample.md as a vault)
  // instead of crashing every Brain / Rewrite call with ENOENT. Status
  // `demo-fixture` lets Brain save, Rewrite and image generation run — all
  // labelled DEMO — until the student connects their own Vault (brand:connect).
  let status=context.status;
  try{await containedFile(vault,context.canonical.company);}
  catch(e){if(e?.code!=='ENOENT')throw e;vault=path.join(brand,'demo');status='demo-fixture';}
  for(const name of ['company','voice','look']){
    const source=status==='demo-fixture'?name+'.md':context.canonical[name];
  ...
  return {owner:status==='demo-fixture'?'阿晴工作室（DEMO）':context.owner,status,outputPlatforms:context.outputPlatforms||["Threads","Newsletter"],revision,documents,samples,assets:manifest.assets,missing:context.missing,websites:context.websites};
}
