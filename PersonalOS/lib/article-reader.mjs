...
export function validateArticleUrl(value,extraSources=[]){
 const url=new URL(value);const host=url.hostname.replace(/^www\./,'');
 const allowed=[...rssConfig.sources,...extraSources].some(s=>{try{return new URL(s.url).hostname.replace(/^www\./,'')===host;}catch{return false;}});
...
export async function readArticle(value,extraSources=[]){
 const url=validateArticleUrl(value,extraSources);
